...
def solution(N, secrets, order_N, orders):
    ...
    for i in range(order_N):
        what_kind= orders.popleft()

        if(what_kind=="I"): # insert
        ...
        elif(what_kind=="D"): # delete
            point= int(orders.popleft())
            len_= int(orders.popleft())
            secrets= secrets[0:point] + secrets[point+len_:] # 삭제 시작점이 인덱스 밖일 경우 고려?

        elif(what_kind=="A"): # append
            len_ = int(orders.popleft())
            for i in range(len_):
                secrets.append(orders.popleft())
        else:
            return "error"
